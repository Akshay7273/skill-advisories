...
export type ArtifactAssessment = {
  ...
  warnings: Array<{ similarTo: string; distance: number }>
  disclaimer: string
}

/** Evaluate an artifact without implying that an absent advisory proves safety. */
export function assessArtifact(feed: Feed, query: ArtifactQuery): ArtifactAssessment {
  const byId = new Map<string, ArtifactAssessment["matches"][number]>()
  ...
  const warnings =
    matches.length === 0
      ? findNearMatches(query.name, collectKnownNames(feed, query.ecosystem)).map(
          ({ name, distance }) => ({ similarTo: name, distance }),
        )
      : []
  return {
    status:
    ...
    warnings,
    disclaimer:
      "No known advisory is not proof of safety. Review provenance, permissions, and source before installation.",
  }
}

export function searchAdvisories(
  feed: Feed,
  options: { query?: string; ecosystem?: Ecosystem; severity?: Severity; limit?: number },
) {
  const needle = options.query?.trim().toLowerCase()
  const limit = Math.min(Math.max(options.limit ?? 20, 1), 50)
  return feed.advisories
    .filter((advisory) => {
      if (options.severity && advisory.severity !== options.severity) return false
      if (
        options.ecosystem &&
