...
export type CompactFeed = {
  ...
  advisories: Array<
    Pick<Advisory, "id" | "type" | "severity" | "artifacts" | "references" | "withdrawn">
  >
}

/** Stable semantic cursor independent of JSON whitespace. */
export function feedCursor(feed: Feed): string {
  return createHash("sha256").update(JSON.stringify(feed)).digest("hex")
}

export function buildFeedDelta(previous: Feed, current: Feed): FeedDelta {
...
export function applyFeedDelta(previous: Feed, delta: FeedDelta): Feed {
  ...
  for (const id of delta.removed) advisories.delete(id)
  for (const advisory of delta.upserts) advisories.set(advisory.id, advisory)
  const next: Feed = {
    ...previous,
  ...
  if (feedCursor(next) !== delta.to) throw new Error("feed delta result failed cursor verification")
  return next
}

export function buildCompactFeed(feed: Feed): CompactFeed {
  return {
    schema_version: "1",
