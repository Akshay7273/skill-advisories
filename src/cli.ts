...
import type { Match } from "./lookup.js"
...
type ParsedArgs = { command?: string; positionals: string[]; json: boolean; feed: string }
...
function report(checked: number, matches: Match[], json: boolean): void {
  if (json) {
    console.log(
      JSON.stringify(
        {
...
if (args.command === "check") {
  if (args.positionals.length === 0) fail("check requires at least one skill name")
